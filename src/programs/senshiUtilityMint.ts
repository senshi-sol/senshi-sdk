...
import { SENSHI_UTILITY_MINT_PROGRAM_ID } from '../utils/constants';
import { sendAndConfirmTransactionWithRetry } from '../utils/helpers';

export class SenshiUtilityMint {
  ...
  async createToken(config: TokenConfig): Promise<PublicKey> {
    ...
    const instruction = new TransactionInstruction({
      ...
      programId: SENSHI_UTILITY_MINT_PROGRAM_ID,
    ...
    console.log(`Utility token created: ${mint.toBase58()}`);
    return mint;
  }
